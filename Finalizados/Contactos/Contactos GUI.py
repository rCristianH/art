...
def eliminar():
    global tContactos
    if tContactos.selection():
        decision = messagebox.askquestion("Eliminando Contacto", "Estás Seguro?")
        if decision == 'yes':
            Contacto.indice = tContactos.index(tContactos.selection())
            Contacto.eliminar()
            messagebox.showinfo('', 'El contacto fue eliminado')
            mostrar()
    else:
        messagebox.showinfo('', 'Debe selecionar un contacto')

def guardar():
    
    Contacto.guardar("Contactos.json")
    messagebox.showinfo('', 'La informacion se guardo en memoria')

def ordenar():
    Contacto.ordenar()
    mostrar()
    messagebox.showinfo('', 'Los contactos fueron ordenados correctamente')
